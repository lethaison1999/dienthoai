...
<div id="phantrang_sp">
	...
	<?php
			...
			//$total_results = mysqli_result(mysqli_query("SELECT COUNT(*) as Num FROM tintuc"),0);  

			// Tính tổng số trang. Làm tròn lên sử dụng ceil()  
			//$total_pages = ceil($total_results / $max_results);  

			$sql1="select matt from tintuc ";
			$query1=mysqli_query($conn,$sql1);
			// $rowss=mysqli_fetch_array($query1);

			$total_results =mysqli_query($conn,"SELECT * FROM 'tintuc' ");  
			

			

			// Tính tổng số trang. Làm tròn lên sử dụng ceil()  
			$total_pages = ceil(intval($query1->num_rows) / intval($max_results));  
